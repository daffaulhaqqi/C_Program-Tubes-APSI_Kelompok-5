...
<body id="register" class="flex items-left text-left min-h-screen">
  <div class="container mx-auto">
    <h1 class="form-underline font-bold text-6xl mt-10 text-center" data-aos="fade-down">#{{ $competitions->id }} | {{ $competitions->name }}</h1>
    <div class="flex flex-row justify-center text-center items-center my-14 mx-[450px]">
        <img src="{{ asset('storage/competition-images/'. $competitions->image) }}" class="w-[345px] h-[510px] object-cover border border-[#d24d49] rounded-2xl" data-aos="fade-down">
        <div class="flex-col w-full ml-16 bg-[#fafafa] px-10 py-6 rounded-2xl border border-[#d24d49]" data-aos="fade-down">
            <form action="{{ route('user.competition.store', $competitions->id) }}" enctype="multipart/form-data" method="POST" class="flex-col text-left align-left justify-left">
            @csrf
